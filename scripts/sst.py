...
DIRFIGS = '/p1-chima/victor/estagio/figs/regional/'
...
for i in range(480, num_tempos):
    ...
    gl = ax.gridlines(crs=ccrs.PlateCarree(), color='black',
                      ...
                      xlocs=np.arange(-180, 180, 30),  # Ajustar intervalo de longitude conforme necessário
                      ylocs=np.arange(-90, 90, 30),  # Ajustar intervalo de latitude conforme necessário
                      draw_labels=True)

    # Definir apenas labels na parte inferior (longitude) e esquerda (latitude)
